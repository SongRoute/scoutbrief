...
LLM 운영 파라미터(LLM_MODEL·LLM_TEMPERATURE)는 src/llm.py 소유 — 레인 간
인터페이스가 아니므로 §1(config.py) 대상이 아니다 (개정 2026-07-10).
CLAUDE.md 규칙 1의 '매직넘버'는 계약 수치를 뜻하며, 명명된 운영 상수 모듈은
이에 반하지 않는다.
...
| runners_group | str | ALL / ON / EMPTY. ON = 투구 시점 on_1b/on_2b/on_3b 중 하나라도 non-null, EMPTY = 전부 null (개정 2026-07-10) |
| pitch_type | str | |
| n | int | |
| usage_pct | float(1) | stand×count_group×runners_group 내 합 100 |
| avg_velo | float(1) | |
| avg_pfx_x, avg_pfx_z | float(2) | |
| xwoba | float(3), nullable | |
| zone_top2 | str | 예: "7,9" |

② 축 규약 (개정 2026-07-10): 두 스플릿 축은 교차하지 않는다 — 모든 행은
count_group=='ALL' 또는 runners_group=='ALL'이다 (둘 다 'ALL'인 행이 기본 아스널).
기존 count_group 스플릿 행은 전부 runners_group=='ALL', 신규 주자 스플릿 행은
count_group=='ALL' × runners_group ∈ {ON, EMPTY}만 존재한다.
개정 이유: 주자 유무별 구종 구성 변화를 리포트에 노출한다. 행 증가는 Verifier
corpus의 숫자 토큰 증가, 곧 §5 층3 우연 일치 표면의 확대를 뜻하며 — §5의 기존
corpus 리스크 수용 문면과 같은 지위의 수용된 리스크다.
...
2) get_pitch_arsenal(pitcher_id: int, season: int = 2026) → 캐시 ②
   (행 스키마 = §3 ② 전체 — runners_group 필드 포함, 개정 2026-07-10.
   source 문면 규약은 공통 반환 래퍼 기존 유지)
...
corpus는 tool_results 전체 직렬화다 — draft의 근거 없는 1~2자리 수가 corpus
어딘가의 수와 우연히 일치하면 통과한다 (수용된 리스크, 개정 세션 2026-07-10 재확인:
corpus를 data 필드로 제한해도 zone·pitches·PA 등 data 내부의 1~2자리 수가 남아
이 클래스를 잡지 못하며, false-fail만 늘린다).
이 검증기는 숫자의 '실재'만 본다 — 해석 방향·표본 귀속 등 의미 오류의 검증은
§6 HITL(분석관 반려) 소관이다.

## 6. 가드레일 스펙 (A레인)
LLM01: 입력 인젝션 패턴 차단(ValueError). 차단 패턴 6종 —
guards.INJECTION_PATTERNS와 문자 단위 일치가 계약 (개정 2026-07-10):
```
(?i)ignore\s+(all\s+)?(previous|prior|above)\s+instructions
(?i)disregard\s+(the\s+)?(system|previous)
이전\s*(지시|명령|프롬프트)\s*(을|를)?\s*무시
(?i)system\s*prompt
(?i)you\s+are\s+now
(?i)</?(system|instruction)>
```
tool_results는 R2로 간접 인젝션 방어.
LLM06: 허용 패턴 우선 검사 — `부상자\s*명단|IL\s*(등재|복귀)|결장` 매치 문장은 통과.
       차단(축소) — `(수술|재활)\s*(부위|일정|경과)|진단`. 문장 단위 마스킹.
       마스킹 토큰 문면: `[의료 상세 마스킹 — LLM06]` — guards.MASK_TOKEN과 문자 단위 일치.
       멱등 조건: 토큰은 ALLOW/BLOCK 어느 패턴에도 매치되지 않아야 한다 —
       guard_output(guard_output(x)) == guard_output(x)의 근거. guards.py의
       모듈 로드 assert가 감지선.
       연봉/사생활 축은 삭제됨 (개정 2026-07-10) — 원천이 statcast 캐시 5종뿐이라
       입력 채널이 없고, 열린 의미 범주는 정규식으로 패턴화할 수 없다.
       rag_notes가 외부 텍스트(뉴스·스카우팅 노트)를 수용하는 시점에 재도입을 검토한다.
guard_output 위치: label_pass 끝(분석관이 마스킹된 최종본을 승인) +
render_deploy 내부(멱등 이중방어, 제거 금지). 이 배치는 with_hitl 경로 한정 —
--linear/--poison(개발 데모)은 배포 산출물이 없어 guard_output 대상이 아니다
(콘솔 draft는 마스킹 미적용).
LLM08: HMAC 승인 토큰. issue_approval_token은 그래프 외부(콘솔)에서만 호출 가능.
미승인 deploy는 PermissionError.
replay(토큰 재사용) 방어는 MVP 범위 밖 — 단일 프로세스 데모 전제,
내용 해시 바인딩(approve-what-you-see)으로 위조·변조는 차단한다.
반려: 콘솔이 토큰을 발급하지 않는 것이 반려다 — 그래프는 interrupt에 잔류하고
배포 산출물은 생성되지 않는다. PermissionError는 반려가 아니라 무효·부재 토큰으로
render_deploy에 진입한 시도에 대한 방어다. 데모: `python run_demo.py --reject`.
...
- bullpen: threat_score 순위표(전원) — 컬럼: 순위/투수(투)/만날 확률 근거/vsL 주무기/vsL xwOBA/개인 전적(PA 병기).
  개인 전적 표기 (개정 2026-07-10): bvp_pa는 0이어도 'N PA'로 표기한다 —
  0 PA는 결측이 아니라 실측값. '-'는 bvp_xwoba가 null일 때 xwOBA 자리에만 쓴다.
  표 상단 소표본 안내 1회. 좌완은 lefty_flag 표기. 이후 상위 위협 투수별 대응 전략(⚑ 추정 라벨).
- gameplan: 이정후 맞춤 공략 포인트. 모든 추정에 ⚑, 전달 금지 문구 포함.
헤더 공통: 출처 각주 [T1]~[T4] + source 메타(live 또는 cache 날짜 YYYY-MM-DD) + 상태 표기.
상태 문면 (개정 2026-07-10) — 승인 전(콘솔 미리보기·--linear): "분석관 승인 대기".
배포본(render_deploy 산출): "분석관 승인 완료 (HMAC 토큰 검증)" —
hitl.APPROVED_STATUS와 문자 단위 일치.